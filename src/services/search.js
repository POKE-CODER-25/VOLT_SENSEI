 ...
 * Returns { items: sortedItems, topConfidence: number, bestMatch: item }
 */
export function intelligentSearch(list, query, keys = ['name'], threshold = 0.4) {
  if (!query || !query.trim()) {
    return { items: list, topConfidence: 0, bestMatch: null };
  }
  ...
  const scored = list.map(item => {
    ...
    for (const key of keys) {
      ...
      // Priority 2: Fuzzy Word Matching
      ...
      const avgWordScore = qWords.length > 0 ? totalWordScore / qWords.length : 0;
  ...
  return {
    ...
    bestMatch: filtered.length > 0 ? filtered[0].item : null
  };
}
